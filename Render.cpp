...
#define normal_ pair<double*,double*>


void figure(int i = 0);
normal_ vectProz(double A[], double B[], double C[]);
...
void Render(OpenGL* ogl)
{
	...
	for (double i = 0; i <= 2 * PI + 0.01; i += 0.01) {
		...
		glTexCoord2d(0.5 + 0.5 * cos(i), 0.5 + 0.5 * sin(i));
		glVertex2f((xc + rad * cos(i)), (yc + sin(i) * rad));
	}
...
void figure(int i)
{
	...
	normal_ N = vectProz(A, A1, H1);

	glNormal3d(N.first[0], N.first[1], N.first[2]);
...
normal_ vectProz(double A[], double B[], double C[])
{
	...
	double point_s[] = { A1[1] * B1[2] - A1[2] * B1[1], A1[2] * B1[0] - A1[0] * B1[2], A1[0] * B1[1] - A1[1] * B1[0] };
	return make_pair(point_s, point_s);
}
